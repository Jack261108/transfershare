...
from utils import print_detailed_error, format_error_info, send_wechat_alert, collect_error, handle_error_and_notify, \
    ErrorCollector, error_collection, mask_cookies
...
class BaiduStorage:
        ...
        with self._client_lock:
            try:
                ...
                if not self._validate_cookies(cookies_dict):
                    ...
                    # 统一错误处理
                    handle_error_and_notify(ValueError(error_msg), "百度网盘客户端初始化失败", self.wechat_notifier,
                                            None, collect=False)
                    return False
                ...
                for retry in range(3):
                    try:
                        ...
                        # 获取超时时间，缺省为 60 秒
                        self.default_timeout = int(os.getenv('BAIDU_REQUEST_TIMEOUT', '60'))
                        ...
                        if retry < 2:
                            ...
                            # 使用统一的错误处理函数
                            handle_error_and_notify(e, "百度网盘客户端初始化最终失败", self.wechat_notifier, None,
                                                    collect=False)
                            return False
            ...
            for seg in prefixes:
                try:
                    ...
                    if 'error_code: 31062' in msg:
                        ...
                        handle_error_and_notify(err, "创建目录失败: 文件名非法", self.wechat_notifier, None,
                                                collect=True)
                        return False
    ...
    def transfer_multiple_shares(self, share_configs, progress_callback=None):
        ...
        with ErrorCollector("批量转存多个分享链接", self.wechat_notifier, None) as ec:
            if not share_configs or not isinstance(share_configs, list):
                ...
                # 统一错误处理
                handle_error_and_notify(ValueError(error_msg), "批量转存配置错误", self.wechat_notifier, None,
                                        collect=True)
                return {
                    'success': False,
                    'error': error_msg,
                ...
                try:
                    ...
                    if result.get('success'):
                        ...
                        if self.wechat_notifier:
                            ...
                            # 聚合策略：仅收集，不即时发送
                            handle_error_and_notify(ValueError(detailed_error), "批量转存单个链接失败",
                                                    self.wechat_notifier, None, collect=True)
    ...
    def transfer_shares_from_text(self, text, default_save_dir=None, progress_callback=None):
        ...
        Returns:
            dict: 批量转存结果
        """
        with ErrorCollector("从文本中解析并批量转存分享链接", self.wechat_notifier, None) as ec:
            try:
                if progress_callback:
                ...
                if not share_configs:
                    ...
                    # 统一错误处理
                    handle_error_and_notify(ValueError(error_msg), "解析分享链接失败", self.wechat_notifier, None,
                                            collect=True)
                    return {
                        'success': False,
                        'error': error_msg,
    ...
    def transfer_share(self, share_url, pwd=None, save_dir=None, progress_callback=None,
        ...
        with ErrorCollector(f"转存分享文件: {share_url}", self.wechat_notifier, None) as ec:
            ...
            try:
                ...
                if not shared_paths:
                    return self._send_error_and_return('获取分享文件列表失败', share_url, None, None,
                                                       "获取分享文件列表失败")
                ...
                # 提取所有文件名（只取文件名部分）
                file_names = [self._normalize_path(f['file_name'], file_only=True) for f in local_files]
                ...
                local_files_dict = {self._normalize_path(f['file_name'], file_only=True): f['md5'] for f in local_files}
                # 步骤3：准备转存（对比文件、准备目录）
                target_dir = save_dir
                is_single_folder = (
                        len(shared_paths) == 1
                ...
                for file_info in shared_files_info:
                    ...
                    clean_normalized = self._normalize_path(clean_path.lstrip('/'), file_only=True)
                    final_normalized = self._normalize_path(final_path.lstrip('/'), file_only=True)
                    # 检查原文件是否存在
                    ...
                    if original_exists or final_exists:
                        ...
                        if src_md5:
                            ...
                            try:
                                ...
                                dest_md5 = None
                                if progress_callback:
                                    progress_callback('info', f'文件已存在，无法获取目标文件 MD5，跳过: {final_path}')
                                continue
                ...
                for _, dir_path, _, _, _ in transfer_list:
                    if dir_path not in created_dirs:
                        if not self._ensure_dir_exists(dir_path):
                            return self._send_error_and_return(f'创建目录失败: {dir_path}', share_url, save_dir, None,
                                                               f"创建目录失败: {dir_path}")
                ...
                if success_count == total_files:  # 全部成功
                    ...
                    return self._send_error_and_return('转存失败，没有文件成功转存', share_url, save_dir, None,
                                                       "转存失败，没有文件成功转存")
